...
if os.path.exists("/data/data/com.termux/files/home/JCR"):
    os.chdir(os.path.expanduser("~"))
    result = subprocess.run(["cp", "-r", "/data/data/com.termux/files/home/JCR", "."], capture_output=True, text=True)
    if result.returncode == 0:
        print("Copied JCR directory successfully. (✅✅✅)")
        time.sleep(10)  
        if os.path.exists(os.path.expanduser("~/JCR")):
            os.chdir(os.path.expanduser("~/JCR"))
            ...
            print("Failed to access JCR directory after 5 seconds delay. (❌❌❌)")
    else:
        print(f"Failed to copy JCR directory: {result.stderr} (❌❌❌)")
else:
    print("Error: JCR directory not found in Termux home. (❌❌❌)")

if os.path.exists(os.path.expanduser("~/JCR")):
    os.chdir(os.path.expanduser("~/JCR"))
else:
    print("Error: JCR directory not found. (❌❌❌)")
    exit(1)
